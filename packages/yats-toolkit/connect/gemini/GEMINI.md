...
1. `search_code("your question", path="__REPO_PATH__")` — ALWAYS start here
2. `find_symbol(name, path="__REPO_PATH__")` on top 2-3 hits
3. `expand_graph([top1, top2], path="__REPO_PATH__")` — callers+callees in one call
4. Only then read files at exact lines from YATS

## Rules
- Max 3-5 YATS calls per task. Synthesize and answer.
